...
def GraphVisualizer(request):
    """
    An http view for visualizing graphs.
    """
    response = HttpResponse()

    import redis
    rc = redis.StrictRedis(host='redis')
    jsonstr = rc.get('test-graph')
    #import ipdb;
    #ipdb.set_trace()
    get = request.GET
    template = get_template('drivers/base2.html')
    html = template.render(Context({'graphjson':jsonstr,'taxonomic_level':'b','grid_level':'c','image_path':'d'}))
    response.content=(html)
    #response.content=str(forest[taxonomic_level])
    response.status_code = 200    
    response.content=(html)
...
def getGraphJsonTest(request):
    """
    SImple service for getting graph json formats
    """
    test = '''[ {"name":"flare.analytics.cluster.AgglomerativeCluster","size":3938,"imports":["flare.animate.Transitioner","flare.vis.data.DataList","flare.util.math.IMatrix","flare.analytics.cluster.MergeEdge","flare.analytics.cluster.HierarchicalCluster","flare.vis.data.Data"]},
